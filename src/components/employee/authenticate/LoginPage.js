...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (isAuthenticated && employee) navigate('/employee');
  });
  ...
  const handleLoginSubmit = async () => {
    ...
    try {
      const login = await employeeLogin(loginData);

      if (login.success) navigate('/employee');
      else {
          ...
          <div className='sub-cont'>
            <div className='img'>
              <div className='img__text m--up'>
                <h2>Employee?</h2>
                <p>Please log in to perform business functions !!!</p>
              </div>
            </div>
          </div>
        </div>
